...
class GenerateurData(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp, list_IDs_label_temp):
        ...
        # Initialization
        aug = A.Compose([
            A.RandomContrast(limit=0.2, p=self.prob),
            A.RandomGamma(gamma_limit=(80, 120), p=self.prob)
            #A.OpticalDistortion(distort_limit=2, shift_limit=0.5, p=self.prob),
            #A.RandomBrightness(limit=0.2, p=self.prob),
            #A.GaussNoise (var_limit=(10.0, 50.0), mean=0, always_apply=False, p=self.prob),
        ...
        for i in range(len(list_IDs_temp)):
            ...
            if self.preprocessing:
                sample = self.preprocessing(image=augmented['image'])
                #X, y = sample['image'], sample['mask']
                X[i,] = sample['image']
                # Store mask
                y[i,] = augmented['mask'][...,0]
            else:
            # Store sample
                X[i,] = augmented['image']
